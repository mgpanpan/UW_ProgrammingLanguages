  ...
  (cond [(var? e) 
        ...
        [(ifgreater? e) 
         (let ([v1 (eval-under-env (ifgreater-e1 e) env)]
           ...
           (cond [(not (int? v1)) (error "e1 does not evaluate to an int")]
                 [(not (int? v2)) (error "e2 does not evaluate to an int")]
        ...
        [(call? e)
         (let ([clo (eval-under-env (call-funexp e) env)]
               [argval (eval-under-env (call-actual e) env)])
           
           (if (closure? clo)
               (let ([newenv (cons (cons (fun-formal (closure-fun clo)) argval) (closure-env clo))]
                     [funname (fun-nameopt (closure-fun clo))])

                 (eval-under-env (fun-body (closure-fun clo))
                                 (if (eq? funname #f)
                                     newenv
                                     (cons (cons funname clo) newenv))))
               (error "Cannot not call object")))]
        [(fst? e)
         (let ([new-e (eval-under-env (fst-e e) env)])
           (if (apair? new-e)
             (apair-e1 new-e)
             (error "fst only works for pairs")))]
        [(snd? e)
         (let ([new-e (eval-under-env (snd-e e) env)])
           (if (apair? new-e)
             (apair-e2 new-e)
             (error "snd only works for pairs")))]
        [(isaunit? e)
